...
<style>
    ...
    /* Streamlitの構造をターゲットにした非常に強力なセレクタ */
    div[data-testid="stAppViewContainer"] section.main div.block-container [data-testid="stVerticalBlock"] > div:last-child {
        ...
        padding: 10px 0 30px 0 !important; /* iPhoneホームバー対策 */
        z-index: 999999 !important;
    }

    /* スマホでも縦並びを阻止し、横一列を強制 */
    div[data-testid="stAppViewContainer"] section.main div.block-container [data-testid="stVerticalBlock"] > div:last-child [data-testid="stHorizontalBlock"] {
...
if st.session_state.page == "作る":
    ...
    uploaded_files = st.file_uploader("写真をアップロード", type=["jpg", "jpeg", "png", "webp"], accept_multiple_files=True)
    ...
    if st.session_state.ingredients_list:
        ...
        if st.button("3. この食材でレシピを生成", use_container_width=True):
...
with nc2:
    ...
    if st.button("確認", key="nav_2", use_container_width=True):
        change_page("確認")
        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
...
with nc3:
    ...
    if st.button("保存", key="nav_3", use_container_width=True):
        change_page("保存")
        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
